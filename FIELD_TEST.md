...
| 11 | OpenCode plugin | `agentbell hooks install opencode`, finish a turn in **any** repo | `run_completed` push **with duration** (plugin is global); exactly one push per turn; a turn under 60 s stays silent (`hook.skipped_short`); a doubled `session.idle` shows up as at most one `hook.skipped_duplicate` record, never as a second buzz — **v1.6.0 result: failed** (24 same-second doubles in 415 turns, no short-turn filter; see the agent table), fixed in v1.6.1. **Re-verification 2026-09-03 (v1.6.2 plugin, OpenCode 1.18.26): short-turn path passed** — one real 7 s turn produced exactly one `hook.skipped_short` record (`duration 7.0`, `min_duration 60.0`), no push, no second record; `verify --agent opencode --since 10m` exit 0. Still open: one real turn ≥ 60 s with the push **and its duration** on the phone |
